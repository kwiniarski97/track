...
const RECENT_WATCH_OLD_GAP_ID = 999208;
...
describe('home page load', () => {
	...
	beforeEach(async () => {
		...
		await db
			...
			.values([
				...
				{ tmdbId: RECENT_WATCH_OLD_GAP_ID, title: 'Old Gap But Recently Watched' }
			])
			.onConflictDoNothing();
		...
		await db.insert(userTracking).values([
			...
			{ userId, mediaType: 'tv', tmdbId: RECENT_WATCH_OLD_GAP_ID, status: 'watching' }
		]);
		...
		await db.insert(episodes).values([
			...
			// Skipped an early episode long ago (still unwatched, aired 400 days back), but
			// caught up on a later one just 5 days ago -- the show is actively being watched
			// despite the stale gap.
			{
				showTmdbId: RECENT_WATCH_OLD_GAP_ID,
				seasonNumber: 1,
				episodeNumber: 1,
				title: 'E1',
				airDate: daysAgo(400)
		...
		await db.insert(userWatches).values([
			...
			},
			{
				userId,
				mediaType: 'tv',
				tmdbId: RECENT_WATCH_OLD_GAP_ID,
				seasonNumber: 1,
				episodeNumber: 2,
				watchedAt: new Date(Date.now() - 5 * 24 * 60 * 60 * 1000)
			}
		]);
	});
	...
	afterEach(async () => {
		...
		await db.delete(episodes).where(eq(episodes.showTmdbId, RECENT_WATCH_OLD_GAP_ID));
	...
	it('buckets watching shows into categories and lists recently watched', async () => {
		...
		expect(result.watching.map((w) => w.tmdbId).sort()).toEqual(
			[MID_RANGE_UNWATCHED_ID, RECENT_WATCH_OLD_GAP_ID].sort()
		);
		...
		// watched cutoff, so only these show up (ordered most-recent-watch first).
		expect(result.recentlyWatched.map((r) => r.tmdbId)).toEqual([
			RECENTLY_WATCHED_ID,
			RECENT_EPISODE_WATCHED_ID,
			RECENT_WATCH_OLD_GAP_ID
		]);
	});
});
